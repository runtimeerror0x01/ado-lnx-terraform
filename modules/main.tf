...
resource "azurerm_container_group" "linux-container-group" {
  ...
  name                = "${var.linux_agents_configuration.agent_name_prefix}"
  location            = var.location
  resource_group_name = var.create_resource_group ? azurerm_resource_group.rg[0].name : data.azurerm_resource_group.rg[0].name
  ip_address_type     = "None"
  os_type             = "Linux"
  subnet_ids = var.enable_vnet_integration ? [data.azurerm_subnet.subnet[0].id] : []
  # network_profile_id  = var.enable_vnet_integration ? azurerm_network_profile.linux_network_profile[count.index].id : null

  dynamic "container" {
  for_each = range(var.linux_agents_configuration.count)
  content {
    name   = "${var.linux_agents_configuration.agent_name_prefix}-${container.key + 1}"
    ...
    # we dont open any ports because it private ip
    # ports {
    #   port     = 80
    #   protocol = "TCP"
    # }
  ...
  dynamic "image_registry_credential" {
    ...
    content {
      ...
      user_assigned_identity_id = var.image_registry_credential.identity
    }
  }
